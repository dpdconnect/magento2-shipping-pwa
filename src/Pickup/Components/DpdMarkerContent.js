import React from "react";
import DpdSelectParcelshopButton from "./DpdSelectParcelshopButton";
import classes from './DpdMarkerContent.css'
import { FormattedMessage } from 'react-intl';
import {func, shape, string} from "prop-types";
import DpdMarker from "./DpdMarker";

const DpdMarkerContent = (props) => {

    const {parcelshop, onParcelshopChange} = props;

    return (
        <div>
            <table>
                <tbody>
                <tr>
                    <td rowSpan={2}>
                        <img className={classes.icon} src="data:image/png;base64,"
                             />
                    </td>
                    <td className={classes.address}  colSpan={3}>
                        <strong>{parcelshop.company}</strong><br />
                        {parcelshop.street} {parcelshop.houseno} <br />
                        {parcelshop.zipcode} {parcelshop.city}
                    </td>
                </tr>
                <tr>
                    <td colSpan={3}>
                        <strong>
                            <FormattedMessage
                                id={'dpdMarkerContent.openingHours'}
                                defaultMessage={'Openingstijden'}
                            />
                        </strong>
                    </td>
                </tr>
                {parcelshop.opening_hours.map(hours => {
                    return (<tr key={hours.weekday}  className={classes.openingHoursRow}>
                            <td></td>
                            <td><strong>{hours.weekday}</strong></td>
                            <td>
                            {hours.open_morning === '00:00' && hours.close_morning === '00:00' ?
                                <FormattedMessage
                                    id={'dpdMarkerContent.closed'}
                                    defaultMessage={'Gesloten'}
                                />
                                 :
                                (hours.open_morning + '-' + hours.close_morning)
                            }
                            </td>
                            <td>
                            {hours.open_afternoon === '00:00' && hours.close_afternoon === '00:00' ?
                                <FormattedMessage
                                    id={'dpdMarkerContent.closed'}
                                    defaultMessage={'Gesloten'}
                                /> :
                                (hours.open_afternoon + '-' + hours.close_afternoon)
                            }
                            </td>
                        </tr>
                    )
                })}
                <tr>
                    <td></td>
                    <td>
                        <DpdSelectParcelshopButton
                            parcelshop={parcelshop}
                            onParcelshopChange={onParcelshopChange}
                        />
                    </td>
                </tr>
                </tbody>
            </table>
        </div>
    )
}

DpdMarkerContent.propTypes = {
    parcelshop: shape({
        parcelshop_id: string,
        company: string,
        street: string,
        postcode: string,
        country: string,
    }),
    onParcelshopChange: func.isRequired
}

export default DpdMarkerContent;
