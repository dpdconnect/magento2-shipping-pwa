import React, {useState} from "react";
import {GoogleMap, useJsApiLoader} from "@react-google-maps/api";
import DpdMarker from "./DpdMarker";
import {mergeClasses} from "@magento/venia-ui/lib/classify";
import defaultClasses from "../pickup.css";
import {array, func, object, string} from "prop-types";
import DpdChosenParcelshop from "./DpdChosenParcelshop";


const DpdMap = (props) => {

    const {
        parcelshops,
        onParcelshopChange,
        apiKey,
        mapContainerStyle
    } = props;

    const { isLoaded } = useJsApiLoader({
        id: 'google-map-script',
        googleMapsApiKey: apiKey
    })
    const [map, setMap] = useState(null)

    const onLoad = React.useCallback(function callback(map) {
        const bounds = new window.google.maps.LatLngBounds();
        parcelshops.map(parcelShop => {
            bounds.extend(new window.google.maps.LatLng(parseFloat(parcelShop.latitude), parseFloat(parcelShop.longitude)));
        })
        map.fitBounds(bounds);
        setMap(map)
    }, [parcelshops])

    const onUnmount = React.useCallback(function callback() {
        setMap(null)
    }, [])

    if(!parcelshops) {
        return null;
    }

    return (
        isLoaded ? <GoogleMap
            mapContainerStyle={mapContainerStyle}
            zoom={10}
            onLoad={onLoad}
            mapTypeId={window.google.maps.MapTypeId.ROAsetParcelshopImageDMAP}
            onUnmount={onUnmount}
            {...props}
        >
            {parcelshops.map(parcelShop => {
                var image = new window.google.maps.MarkerImage(
                    "data:image/png;base64,",
                    new window.google.maps.Size(71, 71),
                    new window.google.maps.Point(0, 0),
                    new window.google.maps.Point(17, 34),
                    new window.google.maps.Size(50, 50)
                );

                return (
                    <DpdMarker
                        key={parcelShop.parcelshop_id}
                        icon={image}
                        parcelshop={parcelShop}
                        map={map}
                        position={{lat: parseFloat(parcelShop.latitude), lng: parseFloat(parcelShop.longitude)}}
                        onParcelshopChange={onParcelshopChange}
                    />)
            })}
        </GoogleMap>
             : <></>
    );
}

DpdMap.propTypes = {
    parcelshops: array,
    onParcelshopChange: func.isRequired,
    apiKey: string,
    mapContainerStyle: object
}

export default DpdMap;
